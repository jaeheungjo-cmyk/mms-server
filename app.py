...
def init_db():
    ...
    # 기본 자재코드
    codes = [
        ('MTR-1001','유도전동기','일반'),('MTR-1002','서보모터','정밀'),
        ('MTR-1003','감속모터','일반'), ('MTR-1004','펌프모터','일반'),
    ]
...
def get_matcodes():
    conn = get_db()
    rows = conn.execute("SELECT * FROM mat_codes").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])
...
def add_matcode():
    d = request.json
    conn = get_db()
    conn.execute("INSERT OR REPLACE INTO mat_codes VALUES (?,?,?)", (d['code'],d['name'],d.get('cat','')))
    conn.commit()
    conn.close()
    return jsonify({'ok':True})
...
def get_matids():
    conn = get_db()
    rows = conn.execute("SELECT * FROM mat_ids").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])

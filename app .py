...
@app.route('/api/motors/<mid>', methods=['DELETE'])
def delete_motor(mid):
    conn = get_db()
    c = conn.cursor()
    # motors.id를 참조하는 history / special_issues 는 FK CASCADE가 걸려있지 않으므로
    # 먼저 연결된 하위 레코드를 지운 뒤 motors 본체를 삭제한다.
    c.execute("DELETE FROM history WHERE motor_id=%s", (mid,))
    c.execute("DELETE FROM special_issues WHERE motor_id=%s", (mid,))
    c.execute("DELETE FROM motors WHERE id=%s", (mid,))
    conn.commit()
    conn.close()
    return jsonify({'ok':True})
